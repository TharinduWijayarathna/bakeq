<div class="pb-28 lg:pb-0">
    <section class="relative overflow-hidden bg-gradient-hero">
        ...
        <div class="relative mx-auto max-w-6xl px-5 py-10 sm:py-14">
            ...
            <h1 class="mt-1 max-w-xl text-4xl leading-[0.95] text-[color:var(--hero-deep)] sm:text-5xl">Tap the look you want</h1>
            <p class="mt-4 max-w-xl text-sm leading-relaxed text-[color:var(--hero-deep)]/80 sm:text-base">
                Choose cake type, tiers, flavour and finish from the cards. Generate builds a preview from your selections — there is no text prompt.
            </p>
            <div class="mt-5 flex flex-wrap gap-2">
                <span class="inline-flex items-center gap-1.5 rounded-full bg-card/80 px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-[color:var(--hero-deep)]">
                    <x-icon name="sparkle" class="size-3.5" /> Visual cards only
                </span>
                <span class="inline-flex items-center gap-1.5 rounded-full bg-card/80 px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-[color:var(--hero-deep)]">
    ...
    <div class="mx-auto max-w-6xl px-5 py-10">
        ...
        <div class="grid items-start gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(18rem,0.85fr)]">
            <div class="space-y-6 transition duration-300" wire:loading.class="pointer-events-none opacity-50" wire:target="generate">
            ...
            <aside class="lg:sticky lg:top-24">
                <div class="overflow-hidden rounded-4xl bg-card shadow-sweet">
                    <div class="flex items-start justify-between gap-3 px-6 pt-6">
                        <div>
                            <p class="font-script text-2xl leading-none text-primary">Studio</p>
                            <h2 class="mt-1 text-2xl">Preview</h2>
                            <p class="mt-1 text-sm text-muted-foreground">{{ $tiers }} {{ $tiers === 1 ? 'tier' : 'tiers' }} · from {{ $settings->formattedBasePrice() }}</p>
                        </div>
                    ...
                    <div class="relative mx-6 mt-5 mb-6 overflow-hidden rounded-3xl bg-secondary ring-1 ring-border/70 lg:mb-0">
                        @if ($design && filled($design->preview_path))
                            <img wire:key="preview-{{ $design->id }}-{{ $design->preview_path }}" src="{{ $design->previewUrl() }}" alt="Generated cake preview" class="h-80 w-full object-cover" wire:loading.class="opacity-40" wire:target="generate">
                        @else
                            <div class="grid h-72 place-items-center px-6 text-center sm:h-80" wire:loading.class="opacity-0" wire:target="generate">
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-muted-foreground">Select the cards, then generate a preview.</p>
                                </div>
                            </div>
                        @endif
                        <x-designer.generating :active="$generating" wire:key="generating-{{ $generating ? 'active' : 'loading' }}" />
                    </div>
                    ...
                    <div class="hidden px-6 pb-6 pt-5 lg:block">
                        <x-designer.generate-button :generating="$generating" />
    ...
    <div class="fixed inset-x-0 bottom-0 z-30 border-t border-border/70 bg-background/95 p-3 backdrop-blur lg:hidden">
        <div class="flex items-center gap-3">
            <div class="min-w-0 flex-1">
                <p class="truncate text-xs text-muted-foreground">{{ $selectedOptions->count() }} selected · {{ $tiers }} {{ $tiers === 1 ? 'tier' : 'tiers' }}</p>
            ...
            @endif
            <x-designer.generate-button compact :generating="$generating" class="shrink-0" />
        </div>
    </div>
</div>
